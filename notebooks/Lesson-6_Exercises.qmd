...
n_muestra1 <- length(aceptacion_muestra_1$id_participante)  # Tamaño de la primera muestra

# Simula la distribución predictiva previa
set.seed(123)
resultados_binomiales <- rbinom(length(theta_samples), size = n_muestra1, prob = theta_samples)
...
# Datos observados
k <- n_aceptaciones_obs  # éxitos, ya calculado anteriormente
n <- nrow(aceptacion_muestra_1)                           # tamaño muestra

# Parámetros de la posterior
alpha_post <- alpha + k  # alpha ya fue definido como 1
beta_post <- beta + n - k  # beta ya fue definido como 1
...
set.seed(123)
resultados_binomiales_post <- rbinom(length(theta_post_samples), size = n_muestra1,  # Ya tenemos theta_post_samples y n_muestra1 definidos previamente
...
k2 <- sum(aceptacion_muestra_2$resp_descarga_app == "Si")  # éxitos
n2 <- nrow(aceptacion_muestra_2)  # tamaño muestra
...
# Parámetros de la posterior
alpha_post2 <- alpha + k2
beta_post2 <- beta + n2 - k2
...
set.seed(123)
resultados_binomiales_post2 <- rbinom(n_sim, size = n2, prob = theta_post_samples2)
...
set.seed(123)
resultados_binomiales_post2_n22 <- rbinom(length(theta_post_samples2), size = n, prob = theta_post_samples2) # n = n muestra 1 (ya calculado)
...
# También n2 (número de observaciones de la muestra 2)

# Simular la predictiva posterior para n = 113
set.seed(123)
resultados_binomiales_post1_n113 <- rbinom(length(theta_post_samples), size = n2, prob = theta_post_samples) # Ya tenemos las muestras de la posterior de theta ajustada con la muestra 1 = theta_post_samples

# Valor empírico de la muestra 2
valor_obs2 <- sum(aceptacion_muestra_2$resp_descarga_app == "Si")

# Calcular el valor-p predictivo posterior
valor_p_pred_2_muestra1 <- mean(resultados_binomiales_post1_n113 >= valor_obs2)
...
cat("- Máximo número de hijos (con título):", max_empirico_con, "\n")
cat("- Máximo número de hijos (sin título):", max_empirico_sin, "\n\n")
...
# max_empirico_con: máximo hijos observado en la muestra con titulación universitaria

# Simular el máximo para cada valor de lambda
max_sim <- map_int(lambda_con, ~ max_poisson(.x, n_con))

# Calcular el valor-p predictivo posterior
p_valor_predictivo <- mean(max_sim <= max_empirico_con)

cat("Valor-p predictivo posterior  P(max ≤", max_empirico_con, ") =",
