...
En este caso usaría un *contraste bilateral*. Como nos preguntamos si dos muestras provienen de la misma población, no es una hipótesis direccional ni nos interesa detectar una desviación en particular. Al final cualquier diferencia significativa (ya sea por exceso o por defecto) es relevante. Además de esta forma se evitan sesgos de interpretación.
$$\text{valor-}p\text{ predictivo posterior}=min[1, 2 × min[P(T(y_rep) ≥ T(y_obs)), P(T(y_rep) ≤ T(y_o_b_s))]]$$
Es decir, el *valor-*$p$ *predictivo posterior* equivaldría al valor mínimo obtenido entre la proporción de simulaciones bajo el modelo ajustado con la muestra 2 (y n = 22) que dieron 17 o más aceptaciones ($P(X ≥ 17)$) y la proporción de simulaciones que dieron 17 o menos aceptaciones ($P(X ≤ 17)$) multiplicado por dos, siempre y cuando el resultado no exceda a uno. Se multiplica por 2 el *valor-*$p$ *predictivo posterior unilateral* ya que en un contraste bilateral, estamos considerando desviaciones en ambas direcciones y por tanto reconocemos que la evidencia contra $h_0$ podría aparecer en cualquiera de las dos colas.

Un *valor-*$p$ *< 0.05* rechazaría la hipotesis nula de que ambas poblaciones son iguales. En este caso *valor-*$p$ *predictivo posterior unilateral* es:

`{r} min(1, 2 * min(prob_mayor_igual, prob_menor_igual))`
...
# Calcular probabilidades:
p_mayor2 <- mean(resultados_binomiales_post1_n113 > n_aceptaciones_obs2)  # Estrictamente mayor
p_menor2 <- mean(resultados_binomiales_post1_n113 < n_aceptaciones_obs2)  # Estrictamente menor
p_igual2 <- mean(resultados_binomiales_post1_n113 == n_aceptaciones_obs2) # Exactamente igual

# Probabilidad de obtener un valor mayor o igual
prob_mayor_igual2 <- p_mayor2 + p_igual2 

# Probabilidad de obtener un valor menor o igual
prob_menor_igual2 <- p_menor2 + p_igual2

# Calcular el valor-p predictivo posterior
valor_p_pred_1_muestra2 <- min(1, 2 * min(prob_mayor_igual2, prob_menor_igual2))
cat("Valor-p predictivo posterior (muestra 2 bajo modelo muestra 1):", valor_p_pred_1_muestra2, "\n")
```

Un valor-p predictivo posterior de `{r} valor_p_pred_1_muestra2` indica que, bajo el modelo ajustado con la muestra 1, hay un `{r} 100 * valor_p_pred_1_muestra2` de probabilidad de observar un número de aceptaciones igual o mayor al observado en la muestra 2.

De nuevo al ser un *valor-*$p$ *> 0.05*, aunque puede indicar cierta discrepancia (es inferior a 0.1) no es suficiente para rechazar $h_0$.
:::
...
# Para cada simulación, determinamos si tiene título y generamos número de hijos
resultados_sim <- tibble(
  tiene_titulo = rbinom(n_sim, 1, prop_titulo_sim),
  tasa_fertilidad = ifelse(tiene_titulo == 1, lambda_con, lambda_sin) # asigna valor lambda_con[i] si tiene_titulo == 1 y lambda_sin[i] si tiene_titulo == 0
)

# Generamos el número de hijos (usualmente Poisson para conteos)
resultados_sim <- resultados_sim %>%
  mutate(n_hijos = rpois(n_sim, tasa_fertilidad)) # Para cada simulación i, generamos un número aleatorio de hijos según la tasa_fertilidad asignada

# Creo que puedo hacer esto de otra forma utilizando las variables que ya tenía con muestras simuladas del número de hijos y_pred_sin e y_pred_con, pero no estoy seguro de cómo...
```
...
# Representación gráfica
ggplot(datos_plot, aes(x = factor(n_hijos), y = prop_con, fill = tipo)) +
