...
uniforme |> # Usando la tabla de datos antes creada, crea un objeto gráfico
  ...
  labs(
    x = "x",
    y = "Densidad",
    title = "Función de Densidad de la Distribución Uniforme"
  )
```
...
normal |>
  ...
  labs(
    x = "x",
    y = "Densidad",
    title = "Función de Densidad de la Normal Estandarizada"
  )
```
...
uniformeEjemplo |> 
  ...
  labs(
    x = "x",
    y = "densidad",
    title = "Función de Densidad de Distribución Uniforme >1"
  )
...
En el caso de la distribución normal, si es la estandarizada (como se dice al inicio), donde media=0 y var=1, el pico es el que se ha dicho (inferior a 1). Si no es estandarizada, si la varianza es más pequeña la curva o campana es más "puntiaguda", pudiéndose superar el valor 1.
...
normalEjemplo |>
  ...
  labs(
    x = "x",
    y = "Densidad",
    title = "Función de Densidad de la Normal Estándar >1"
  )
...
beta_11 |>
  ...
  labs(
    x = "x",
    y = "densidad",
    title = "Función de distribución de Beta(1,1)"
  )
```
...
- Cuando α > β, la distribución es asimétrica negativa o izquierda.
- Cuando α < β, la distribución es asimétrica positiva o derecha.
...
Cuando son distintas la distribución es asimétrica izquierda (cuando α > β) o asimétrica derecha (cuando α < β).
...
valor_esperado_analitico <- posterior_alpha / (posterior_alpha + posterior_beta)
...
# Calcular valor esperado de forma numérica
valor_esperado_numerico <- posterior_densidad |>
  mutate(
    # Para cada punto, multiplicamos theta por su densidad y por PREC
    producto = theta * densidad * PREC
  ) |>
  summarise(
    # Sumamos todos los productos para obtener la integral
    valor_esperado = sum(producto)
  ) |>
  pull(valor_esperado)
...
cat("Valor esperado analítico:", valor_esperado_analitico, "\n")
cat("Valor esperado numérico:", valor_esperado_numerico, "\n")
