...
```{r representacion-grafica-discreta}
...
```{r exploracion-datos-logit}
# Cargar el paquete mcmc
library(mcmc)

# Cargar el dataset logit
data(logit)
...
logit_graf <- pivot_longer(logit, 
                           cols = everything(),
                           names_to = "Variable", 
                           values_to = "Valor")

# Boxplots para detectar outliers
ggplot(logit_graf, aes(x = Variable, y = Valor, fill = Variable)) +
  geom_boxplot() +
  labs(title = "Distribución de variables y detección de outliers",
       x = "Variable",
       y = "Valor")

# Histogramas para ver la distribución de cada variable
ggplot(logit_graf, aes(x = Valor, fill = Variable)) +
...
*Logit* es un marco de datos con 100 observaciones de 5 variables: una variable *y* de respuesta binaria (0 o 1) y *x1*,*x2*,*x3*,*x4* como variables predictoras numéricas.  
- La **variable respuesta *y*** presenta aproximadamente un 55% de casos donde *y=1*.  
- Todas las **variables predictoras** tienen distribuciones aproximadamente normales, centradas cerca de 0. Hay algunos valores atípicos en las variables, especialmente en *x2* y en *x3*. Todas tienen correlaciones positivas con la variable respuesta (siendo las mayores x1 y x2 con 0.503 y 0.513 respectivamente) y algunas correlaciones moderadas entre ellas.  
...
```{r ajustar-ML-Geyer}
# Ajustar el modelo de regresión logística frecuentista
...
$$\log\left(\frac{p}{1-p}\right) = 0.633 + 0.739 \cdot x_1 + 1.114 \cdot x_2 + 0.478 \cdot x_3 + 0.694 \cdot x_4$$
  
Donde $p$ es la probabilidad de que $y = 1$.  

En cuanto los resultados particulares:  
- Intercepto (0.633): Cuando todas las variables predictoras son cero, el logaritmo de las probabilidades (log-odds) es 0.633, lo que corresponde a una probabilidad de $\frac{e^{0.633}}{1+e^{0.633}} \approx 0.653$ de que $y = 1$.  
- Los coeficientes, por otra parte, indican el aumento del logaritmo de las probabilidades por cada aumento de esa unidad $x$ en particular, manteniendo las demás variables constantes. Estos son todos positivos (lógico, teniendo en cuenta la tabla de correlaciones) y varían entre 0.478 para $x_3$ y 1.114 para $x_2$. Las variables $x_1$ y $x_2$ son estadísticamente significativas al nivel de 0.05, mientras que la variable $x_4$ es marginalmente significativa al nivel de 0.10 y la variable $x_3$ no es estadísticamente significativa, por lo que no hay suficiente evidencia de que este efecto positivo de la variable es diferente de cero.  
- La bondad de ajuste con una reducción de la devianza de 49.96 y un AIC de 97.67 sugieren un buen ajuste de los datos y que el modelo explica una cantidad sustancial de la variabilidad en los datos.  
:::
...
```{r estructura-datos_logit}
# Visualizar estructura de datos_logit
str(datos_logit)
```
*datos_logit* almacena una lista para que pueda ser leida y utilizada por la función *stan()* para ajustar el modelo, donde la sección *data* del archivo "src/geyer_2011_logistic.stan" nos especifica que:  
- *y*: Vector de valores de la variable respuesta correspondiente donde se cumple *int<lower=0, upper=1>* (de longitud *N*).  
- *x*: Matriz de variables predictoras donde *matrix[N, K]* (con *N* columnas y *K* filas).  
- *N*: Número de casos u observaciones en los datos ha de ser *int<lower=0>*.  
- *K*: Número de variables predictoras, también *int<lower=0>*.
:::
...
```{r fit_logit_stan}
# Mostrar estructura de fit_logit_stan
print(fit_logit_stan)
```
  
Esas líneas muestran los componentes principales del modelo, donde se ve:
- *anon_model* es el **nombre genérico** que Stan asigna al modelo cuando no se especifica un nombre explícito.  
- Se ejecutaron *4* **cadenas (*chains*) independientes** o 4 secuencias de muestras que comienzaron desde un punto inicial diferente.  
- Cada cadena realizó *1000* **iteraciones** en total.
- Las primeras *500* iteraciones de cada cadena se utilizaron como **warmup** o "burn-in". Estas muestras iniciales según la función *stan()* se descartan porque la cadena aún no ha alcanzado su distribución estacionaria (posterior) y el algoritmo está ajustando sus parámetros adaptativos durante este periodo.  
- El **factor de adelgazamiento (*thin*)** es *1*, lo que significa que se conservan todas las muestras post-warmup. Si fuera mayor (por ejemplo, 2), solo se guardaría 1 de cada 2 muestras para reducir la autocorrelación.  
- Por tanto cada cadena contribuye con 500 muestras después del periodo de calentamiento, lo que equivale a un **Total de muestras post-warmup** de *2000* para estimar la distribución posterior.  
  
Significado de la columna *se_mean*:  
La columna se_mean representa el error estándar de la media (Standard Error of the Mean) que en contexto sería interpretado como el "error estándar de Monte Carlo" (MCSE). A diferencia del Monte Carlo Ordinario (OMC), donde las muestras son independientes e idénticamente distribuidas (i.i.d.) en MCMC las muestras están correlacionadas, lo que afecta al error estándar. La fórmula correcta es::
$$\text{SE}{\text{MCMC}} = \frac{\sigma{\text{efectiva}}}{\sqrt{n_{\text{efectivo}}}}$$, donde:
- $\sigma_{\text{efectiva}}^2 = \sigma^2 + 2\sum_{k=1}^{\infty}\text{cov}(g(X_i), g(X_{i+k}))$  
- $n_{\text{efectivo}}$ es el tamaño efectivo de muestra, que es menor que $n$ debido a la autocorrelación
  
Por tanto esta medida de error tiene en cuenta la autocorrelación entre las muestras y depende del tamaño efectivo de la muestra ya que, cuanto mayor sea la autocorrelación en la cadena, menor será el tamaño efectivo de la muestra y mayor será el *se_mean*.   
  
Indica cuanto podría variar nuestra estimación de la media posterior si repitieramos el proceso de MCMC, por lo que, contra menor sea el valor de este parámetro, mayor confianza se puede tener en las estimaciones de las medias posteriores. Si en cambio es alto tenemos que plantearnos si hemos ejecutado suficientes iteraciones para estimar con precisión la media posterior o si necesitamos ajustar el algoritmo MCMC para reducir la autocorrelación.   
  
En los resultados de *fit_logit_stan* vemos que el parámetro *se_mean* es realmente bajo, lo que indica una buena estimación de las medias posteriores.
:::
...
1. **Número de cadenas**: *stan()* puede ejecutar cadenas independientes en paralelo especificadas en el argumento *chains* (4 por defecto), lo que permite evaluar la convergencia mediante la comparación entre cadenas mientras que *Greyer(2011)* utiliza principalmente una cadena, aunque menciona la posibilidad de continuar la cadena con ejecuciones adicionales (*out2 <- metrop(out, x = x, y = y)*).  
2. **Iteraciones**: *stan()* utiliza por defecto 2000 iteraciones (incluyendo las de calentamiento), modificables en *iter*. En el texto, *Geyer(2011)* especifica las iteraciones iniciales con el parámetro 1e3 a través del tercer argumento (*out <- metrop(lupost, beta.init, 1e3, x = x, y = y)*). Después de ajustar la escala para obtener una dasa de aceptación adecuada, Geyer ejecuta una cadena más larga con 10,000 iteraciones. Finalmente para el cálculo de las medias y varianzas posteriores utiliza un enfoque de lotes (*outfun = function(z, ...) c(z, zˆ2), x = x, y = y)*) donde especifica 100 lotes (*nbatch = 1e2*) y que cada lote contiene 100 iteraciones (*blen = 100*).  
3. **Burn-in**: *stan()* puede descartar explícitamente las primeras iteraciones especificandolas en el argumento *warmup* (argumento que por defecto es *iter/2*). Contrariamente a esto *Geyer(2011)* argumenta que el burn-in es un concepto absurdo y no es necesario si se inicia desde un punto de alta probabilidad (como el MLE).  
4. **Thinning**: Si nos referimos estríctamente a la técnica de conservar solo una fracción de las muestras generadas, descartando las demás a intervalos regulares, mientras que *stan()* especifica elxplícitamente el thinning mediante el parámetro *thin* (p.e. *thin=10* significa que guardamos 1 de cada 10 muestras), *Geyer(2011)* no especifica un parámetro específico para el thinning. En lugar de usar esta estratégia, *Geyer(2011)* aborda la autocorrelación mediante su análisis (examina los gráficos de autocorrelación para determinar hasta qué punto las muestras estan correlacionadas) y el uso de lotes descrito anteriormente, además de tener en cuenta esta en el análisis, a través de los MCMCSE.  
5. **Valores iniciales**: *stan()* utiliza un valor inicial aleatorio (a no ser que se especifique lo contrario a través del argumento *seed*), lo que, junto con la posibilidad de ejecutar más de una cadena, le permite una mejor evaluación de la convergencia. Por su parte *Geyer(2011)* inicia la cadena en la estimación de máxima verosimilitud (*beta.init <- as.numeric(coefficients(out))*), aunque reconoce que sería más natural para un método Bayesiano usar "the posterior mode rather than the maximum likelihood estimate".  
:::
...
Las muestras del modelo aproximado con Stan representan adecuadamente la distribución posterior de los parámetros del modelo, ya que:  
- **Todos los valores de Rhat son 1.00**. Rhat mide la convergencia de múltiples cadenas donde valores cercanos a 1 son mejores.  
- **Hay un alto n_eff** que indica cuántas muestras independientes equivalentes tenemos (e indirectamente la autocorrelación, que será más baja contra más alto sea el tamaño efectivo). En este caso para los parámetros principales, alpha y betas, el n_eff > 200; incluso para lp__, n_eff=937, que es sustancial. En general se acepta un n_eff > 100.  
- **Error Estándar de Monte Carlo de 0.01 para los coeficientes**. Ya se ha justificado el significado e interpretación de *se_mean*.  
- Adicionalmente, los resultados son consistentes con el modelo frecuentista examinado en la pregunta 13.  
:::
...
**Equivalencia de parámetros entre modelos (Modelo Frecuentista, Modelo Bayesiano (stan) y descripción de la variable):**    
- *(Intercept)* = alpha = Intercepto del modelo  
- *x1* = *beta[1]* = Coeficiente para la variable x1  
- *x2* = *beta[2]* = Coeficiente para la variable x2  
- *x3* = *beta[3]* = Coeficiente para la variable x3  
- *x4* = *beta[4]* = Coeficiente para la variable x4  
  
Los valores de los parámetros son muy similares entre ambos modelos, lo que indica que las distribuciones previas utilizadas en el modelo bayesiano son débilmente informativas y permiten que los datos dominen las estimaciones.  
  
Sin embargo las estimaciones bayesianas son ligeramente mayores a las frecuentistas en todos los casos. Esto puede deberse a las influencias de las distribuciones previas (normal con media 0 y DE 2), posibles diferencias en los algoritmos que las dos funciones utilizan para la estimación o una asimetría en las distribuciones posteriores. Adicionalmente el modelo con *stan()* aporta incertidumbre a través de las distribuciones posteriores, mientras el frecuentista sólo proporciona errores estándar y valores p.  
:::
...
```{r representacion-grafica-stan}
# Visualización usando también ggplot
  plot(fit_logit_stan) +
  labs(
    title = "Distribuciones posteriores de los parámetros",
    subtitle = "Modelo de regresión logística bayesiana",
    x = "Valor del parámetro",
    y = "Densidad"
  )
```
  
El gráfico muestra las distribuciones posteriores de los parámetros del modelo de regresión logística bayesiana con intérvalos de credibilidad del 80% (nivel interno) y 95% (nivel externo).  
  
- Las **barras rojas** (más gruesas) representan los intérvalos de credibilidad del 80% (*ci_level:0.8*). Esto significa que hay un 80% de que el verdadero valor del parámetro se encuentre dentro de este intérvalo.  
- Las **líneas más delgadas** representan el IC del 95% (*outer_level:0.95*).  
- Los **puntos negros** representan las estimaciones puntuales de la mediana posterior de cada parámetro.  
  
Esto permite visualizar de forma clara no solo las estimaciones puntuales sino también la incertidumbre asociada a cada parámetro.
:::
...
# Extraer las muestras posteriores
posterior_samples <- as.matrix(fit_logit_stan)

# Visualizar las trazas de las cadenas para los parámetros principales
mcmc_trace(posterior_samples, pars = c("alpha", "beta[1]", "beta[2]", "beta[3]", "beta[4]"))
```

Las trazas representan cómo los valores de cada parámetro han cambiado a lo largo de las iteraciones post-calentamiento del algoritmo MCMC. Cada punto del eje X representa una iteración, mientras que el eje Y representa el valor del parámetro estimado para esa iteración. He puesto los gráficos en una misma visualización porque entiendo que tampoco estás pidiendo una interpretación exhaustiva de lo que sucede, si no que demos la idea general del significado de estas.  
  
A nivel general, todas las trazas parecen fluctuar alrededor de valores constantes sin tendencias claras (lo que sugiere que las cadenas alcanzan la estacionariedad) y parecen explorar adecuadamente el espacio de parámetros, moviéndose libremente entre todos los valores que se consideran plausibles. *beta[3]* (y menos consistentemente *beta[4]* fluctúa con valores negativos y ocasionalmente toma valores cercanos a cero, lo que sería consistente con el modelo frecuentista que no es capaz de descartar que este valor sea 0 y que esta variable pueda ser prescindible.  
:::
...
# Ajustar el modelo frecuentista para obtener valores iniciales
out <- glm(y ~ x1 + x2 + x3 + x4, data = logit,
           family = binomial(), x = TRUE)

# Extraer la matriz de diseño y la variable respuesta
x <- out$x
y <- out$y

# Definir la función de log-verosimilitud posterior
...
# Valores iniciales (coeficientes del modelo frecuentista)
beta.init <- as.numeric(coefficients(out))

# Ejecutar el algoritmo con una escala adecuada
# Primero ajustamos la escala para obtener una tasa de aceptación adecuada
out <- metrop(lupost, beta.init, 1e3, x = x, y = y)
out$accept  # Verificar tasa de aceptación

# Ajustar la escala si es necesario
out <- metrop(out, scale = 0.4, x = x, y = y)
out$accept  # Verificar tasa de aceptación mejorada

# Ejecutar una cadena más larga
out <- metrop(out, nbatch = 1e4, x = x, y = y)

# Definir la función outfun que devuelve los parámetros y sus cuadrados
outfun <- function(z, ...) c(z, z^2)

# Ejecutar con la función outfun
out <- metrop(out, nbatch = 1e2, blen = 100,
              outfun = outfun, x = x, y = y)

# Extraer los resultados
# Las primeras 5 columnas son los parámetros originales
# Las siguientes 5 columnas son los parámetros al cuadrado
params <- apply(out$batch, 2, mean)
params_se <- apply(out$batch, 2, sd) / sqrt(out$nbatch)

# Separar los resultados
mu <- params[1:5]  # Medias de los parámetros
mu_se <- params_se[1:5]  # Error estándar de las medias

# Calcular varianzas posteriores
# Usando la fórmula Var(θ) = E(θ²) - E(θ)²
sigmasq <- params[6:10] - params[1:5]^2

# Calcular el error estándar de las varianzas usando el método delta
# (Ver Geyer 2011, p. 33 para los detalles)
u <- out$batch[, 1:5]  # Parámetros originales
v <- out$batch[, 6:10]  # Parámetros al cuadrado
ubar <- apply(u, 2, mean)
vbar <- apply(v, 2, mean)
deltau <- sweep(u, 2, ubar)
deltav <- sweep(v, 2, vbar)
foo <- sweep(deltau, 2, ubar, "*")
sigmasq_se <- sqrt(apply((deltav - 2 * foo)^2, 2, mean) / out$nbatch)

# Crear un data frame con los resultados
resultados <- data.frame(
  Parametro = c("Intercepto", "x1", "x2", "x3", "x4"),
  Media = mu,
  SE_Media = mu_se,
  Varianza = sigmasq,
  SE_Varianza = sigmasq_se,
  DE = sqrt(sigmasq)
)
...
muestras <- out$batch[, 1:5]
colnames(muestras) <- c("Intercepto", "x1", "x2", "x3", "x4")

# Convertir a formato largo para ggplot usando R base
muestras_df <- as.data.frame(muestras)
muestras_long <- data.frame(
  Parametro = rep(names(muestras_df), each = nrow(muestras_df)),
  Valor = unlist(muestras_df)
)

# Crear gráfico de densidades con ggplot2
library(ggplot2)
ggplot(muestras_long, aes(x = Valor, fill = Parametro)) +
  geom_density(alpha = 0.5) +
  facet_wrap(~ Parametro, scales = "free") +
  labs(title = "Distribuciones posteriores de los parámetros",
       x = "Valor", y = "Densidad")
```
